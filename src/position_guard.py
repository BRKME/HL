...
# 16.09: горизонт выхода выровнен со входом. Оператор: «ставка не на часы,
# а на недели».
#
...
#
# 24 прогона = двое суток: достаточно, чтобы пережить суточный шум, и
# вдвое меньше самого короткого рабочего окна из перебора. Стоп при этом
# остаётся немедленным — он про риск, а не про горизонт.
FLIP_CONFIRM_RUNS = 24
...
# (до 02.08), 2 = гистерезис 6 часов, 3 = гистерезис двое суток (с 16.09,
# выравнивание горизонта выхода со входом). Выборки версий 2 и 3 нельзя
# складывать: это разные стратегии.
POLICY_VERSION = 3
