...
def format_for_digest(accounts_ratio: Optional[float],
    ...
    # «крупные короче» — калька с внутреннего термина: «короче» по-русски
    # про длину, а не про сторону сделки, и пометка читалась как чепуха
    # (замечание оператора 08.09). Говорим, что происходит: толпа в лонге,
    # а крупные деньги в шорте. Это единственный признак с устойчивым
    # знаком — −0.45% на семи монетах из девяти.
    note = ""
    if la is not None and lt is not None and (la - lt) >= 0.15:
        note = " ⚠️ крупные против толпы"
